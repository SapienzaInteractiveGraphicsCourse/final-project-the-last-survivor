...
export class Avatar {
  ...
  init() {
    ...
    loader.load('/Assets/ak_47_with_hands_and_animations/scene.gltf', (gltf) => {
      ...
      const modelPosition = new THREE.Vector3(0.3, -1.4, -0.4); // Adjust the position as needed
      ...
      this.model.scale.set(0.08, 0.08, 0.08); // Adjust the scale values as needed
  
      // Set renderOrder for the AK47 model
      this.model.renderOrder = 1; // Set the renderOrder to render above other objects
  
      // Set renderOrder for each child mesh in the AK47 model
